...
export default function VoiceCoachPage() {
  ...
  const startSession = async () => {
    ...
    try {
      const newRoomName = `coach-${user.uid}-${Date.now()}`;
      const identity = `user-${user.uid}`;

      // Get the display name from profile or Firebase user
      const displayName = profile?.displayName || user.displayName || "Athlete";
      console.log("[Coach] Starting session with name:", displayName, "profile:", profile);
      ...
      const tokenResponse = await getToken(newRoomName, identity, userContext);
  ...
  // Stable error handler for LiveKit
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <GlassCard className="mb-6">
          ...
          <AnimatePresence mode="wait">
            ...
            {connectionState === "connected" && liveKitUrl && liveKitToken && roomName && (
              <motion.div
                key="connected"
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
